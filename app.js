...
//     var geoData = topojson.feature(mapData, mapData.objects.London_Ward.geometries).features;
...
d3.json('./london-topojson1.json', function(error, mapData) {
  if (error) throw error;
  console.log(mapData.objects.london_geo);
  //   var path = d3.geoPath();
  var width = 600;
  var height = 600;

  var geoData = topojson.feature(mapData, mapData.objects.london_geo).features;

  //   var geoData = topojson.feature(mapData, mapData.objects.countries).features;


  var projection = d3.geo.mercator()
  .center([0.1248, 51.5074])
    .scale(30000)
